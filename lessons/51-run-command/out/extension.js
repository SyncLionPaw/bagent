...
async function openBagentPanel() {
    ...
    await vscode.commands.executeCommand("bagent51.chat.focus");
}
function activate(context) {
    const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? context.extensionPath;
    const config = vscode.workspace.getConfiguration("bagent51");
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("bagent51.chat", sidebar, {
        webviewOptions: { retainContextWhenHidden: true },
    }), vscode.commands.registerCommand("bagent51.open", () => openBagentPanel()));
}
function deactivate() {
    agent?.shutdown();
    agent = null;
