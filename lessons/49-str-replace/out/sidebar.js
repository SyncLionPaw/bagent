...
<body>
  <div class="hint" id="hint">str_replace 局部修改 · read_file 行号 · 模型 ${currentModel} · /help</div>
  <div id="log"></div>
  <form id="f">
    ...
    window.addEventListener("message", (e) => {
      ...
      if (msg.type === "event") {
        ...
        if (ev.type === "ToolCallPending") {
          ...
          pendingEl.className = "pending" + (ev.name === "write_file" || ev.name === "str_replace" ? " danger" : "");
          ...
          pendingEl.innerHTML =
            ...
            '<span class="pending-args">(' + argsEsc + ')</span>' +
            '<span class="pending-hint"> Enter 允许 · Esc 拒绝</span></div>' +
            '<div class="actions">' +
...
class ChatSidebarProvider {
    ...
    resolveWebviewView(webviewView) {
        ...
        void this.agent.sessionReady.then((info) => {
            if (info.restored) {
                this.postSys(webviewView.webview, `已恢复上次会话（${info.messages} 条消息）\n${info.path ?? ""}`);
            }
            else {
                this.postSys(webviewView.webview, `新会话 · 自动保存至 ~/.bagent/.../sessions/\n会话 ID：${info.sessionId}`);
            }
        });
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            ...
            if (action.type === "save_chat") {
                try {
                    ...
                    if (result.ok) {
                        this.postSys(webviewView.webview, `已保存对话「${result.name}」\n${result.messages} 条消息 → ${result.path}`);
                    }
                    else {
            ...
            if (action.type === "load_chat") {
                try {
                    ...
                    if (result.ok) {
                        this.postSys(webviewView.webview, `已加载「${result.name}」\n${result.messages} 条消息 → ${result.path}\n（继续对话将基于已恢复 history）`);
                    }
                    else {
                        webviewView.webview.postMessage({ type: "error", message: result.error });
                    }
