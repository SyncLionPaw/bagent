...
async function openBagentPanel() {
    ...
    await vscode.commands.executeCommand("bagent49.chat.focus");
}
function activate(context) {
    const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? context.extensionPath;
    const config = vscode.workspace.getConfiguration("bagent49");
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("bagent49.chat", sidebar, {
        webviewOptions: { retainContextWhenHidden: true },
    }), vscode.commands.registerCommand("bagent49.open", () => openBagentPanel()));
}
function deactivate() {
    agent?.shutdown();
    agent = null;
