...
function toolsParagraph(): string {
  const local =
    "本地工具：ask_user_question、pwd、ls、stat_file、read_file、grep、plan_operate、write_file、str_replace、delete_file、run_command。读用 read_file/grep/ls/pwd；写删用 str_replace/write_file/delete_file（本课只产出编辑提案 JSON，不落盘）；探测 URL/进程/端口用 run_command（仅 curl/ps/lsof）。多步任务用 plan_operate。写删与 run_command 须用户批准。";
