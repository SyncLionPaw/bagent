...
import { compactProposalSummary, isEditTool, parseEditProposal } from "./editProposal.js";
import {
  ASK_USER_MAX_CHARS,
  ...
  async *turn(
    ...
    while (true) {
      ...
      for (const call of assistant.tool_calls) {
        ...
        let originalLength = result.originalLength;
        if (isEditTool(name)) {
          const proposal = parseEditProposal(result.output);
          if (proposal) {
            toolOutput = compactProposalSummary(proposal);
            truncated = false;
