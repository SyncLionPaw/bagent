...
async function openBagentPanel() {
    ...
    await vscode.commands.executeCommand("bagent52.chat.focus");
}
function activate(context) {
    const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? context.extensionPath;
    const config = vscode.workspace.getConfiguration("bagent52");
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("bagent52.chat", sidebar, {
        webviewOptions: { retainContextWhenHidden: true },
    }), vscode.commands.registerCommand("bagent52.open", () => openBagentPanel()));
}
function deactivate() {
    agent?.shutdown();
    agent = null;
