...
export class CalculatorPanelProvider implements vscode.WebviewViewProvider {
  resolveWebviewView(webviewView: vscode.WebviewView): void {
    webviewView.webview.options = { enableScripts: true };
    webviewView.webview.html = getHtml();

    webviewView.webview.onDidReceiveMessage((msg: { type: string; expr?: string }) => {
      if (msg.type !== "eval" || msg.expr === undefined) return;
      try {
        const value = evalExpr(msg.expr);
