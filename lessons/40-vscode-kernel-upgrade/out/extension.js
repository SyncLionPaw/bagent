...
async function openBagentPanel() {
    ...
    await vscode.commands.executeCommand("bagent40.chat.focus");
}
function activate(context) {
    const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? context.extensionPath;
    const config = vscode.workspace.getConfiguration("bagent40");
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("bagent40.chat", sidebar, {
        webviewOptions: { retainContextWhenHidden: true },
    }), vscode.commands.registerCommand("bagent40.open", () => openBagentPanel()));
}
function deactivate() {
    agent?.shutdown();
    agent = null;
