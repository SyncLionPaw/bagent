...
async function openBagentPanel() {
    ...
    await vscode.commands.executeCommand("bagent54.chat.focus");
}
function activate(context) {
    (0, diffPreview_1.registerDiffContentProvider)(context);
    (0, diffPreview_1.registerEditCommands)(context);
    const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? context.extensionPath;
    const config = vscode.workspace.getConfiguration("bagent54");
    ...
    const proc = new spawn_1.AgentProcess();
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("bagent54.chat", sidebar, {
        webviewOptions: { retainContextWhenHidden: true },
    }), vscode.commands.registerCommand("bagent54.open", () => openBagentPanel()));
}
function deactivate() {
    agent?.shutdown();
    agent = null;
