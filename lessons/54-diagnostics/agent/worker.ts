...
let editApplyResolve: ((allow: boolean) => void) | null = null;
...
rl.on("line", async (line) => {
  ...
  if (req.op === "edit_apply") {
    editApplyResolve?.(req.allow);
    editApplyResolve = null;
    return;
  }
  ...
  busy = true;
  try {
    for await (const event of agent.turn(req.message.trim(), approve, askUser, editApply)) {
      emit(event);
    }
    persistSession();
  } catch (err) {
