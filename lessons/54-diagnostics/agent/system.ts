...
function toolsParagraph(): string {
  const local =
    "本地工具：ask_user_question、pwd、ls、stat_file、read_file、grep、plan_operate、write_file、str_replace、delete_file、run_command。读用 read_file/grep/ls/pwd；写删用 str_replace/write_file/delete_file（先在编辑器 diff 预览，Accept 才写盘）；探测 URL/进程/端口用 run_command（仅 curl/ps/lsof）。多步任务用 plan_operate。run_command 须侧栏审批。";
