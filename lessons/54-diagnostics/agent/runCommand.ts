...
export function tokenizeCommand(command: string): string[] {
  ...
  if (quote) throw new Error("命令字符串引号未闭合");
...
export function validateCommandWhitelist(argv: string[]): WhitelistResult {
  if (!argv.length) {
...
function validateCurl(args: string[]): WhitelistResult {
...
function validatePs(_args: string[]): WhitelistResult {
  return { ok: true, program: "ps", args: _args };
}

function validateLsof(args: string[]): WhitelistResult {
...
function validateTsc(args: string[]): WhitelistResult {
...
function validateNodeCheck(args: string[]): WhitelistResult {
...
function validatePython(args: string[]): WhitelistResult {
  ...
  return {
    ok: false,
    reason:
      "python3 只允许 `-m py_compile <file>`（语法检查）或 `-c <expr>`（求值）",
  };
}

export function checkCommandString(command: string): WhitelistResult {
...
function formatOutput(stdout: string, stderr: string, exitCode: number | null, signal: string | null): string {
  const parts: string[] = [];
