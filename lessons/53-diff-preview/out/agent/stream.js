...
const reminder_js_1 = require("./reminder.js");
...
function messagesForApi(history) {
    ...
    return history.map((msg, i) => {
        ...
        if (i !== lastUserIdx)
            return msg;
        return { ...msg, content: (0, reminder_js_1.augmentUserMessage)(msg.content, turnIndex) };
    });
}
async function* streamEvents(history) {
    let res;
    try {
    ...
    if (thinkingOpen)
        yield { type: "ThinkingEnd" };
    const tool_calls = toolParts.filter((t) => t.id && t.function?.name);
    return {
        role: "assistant",
