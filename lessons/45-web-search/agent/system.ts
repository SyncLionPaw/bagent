...
function toolsParagraph(): string {
  const local =
    "本地工具：ask_user_question、pwd、ls、stat_file、read_file、grep、write_file。需要用户二选一、指定文件名、确认方案等时，先 ask_user_question，不要 ls 完再用文字问。ask_user_question 用户可跳过，你会收到 hint 后自行决定。path 须绝对路径且在 cwd 下。write_file 须用户批准。";
