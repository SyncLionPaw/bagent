...
async function openBagentPanel() {
    ...
    await vscode.commands.executeCommand("bagent45.chat.focus");
}
function activate(context) {
    const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? context.extensionPath;
    const config = vscode.workspace.getConfiguration("bagent45");
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("bagent45.chat", sidebar, {
        webviewOptions: { retainContextWhenHidden: true },
    }), vscode.commands.registerCommand("bagent45.open", () => openBagentPanel()));
}
function deactivate() {
    agent?.shutdown();
    agent = null;
