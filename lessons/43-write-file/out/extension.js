...
async function openBagentPanel() {
    ...
    await vscode.commands.executeCommand("bagent43.chat.focus");
}
function activate(context) {
    const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? context.extensionPath;
    const config = vscode.workspace.getConfiguration("bagent43");
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("bagent43.chat", sidebar, {
        webviewOptions: { retainContextWhenHidden: true },
    }), vscode.commands.registerCommand("bagent43.open", () => openBagentPanel()));
}
function deactivate() {
    agent?.shutdown();
    agent = null;
