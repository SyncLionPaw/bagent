...
async function openBagentPanel() {
    ...
    await vscode.commands.executeCommand("bagent47.chat.focus");
}
function activate(context) {
    const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? context.extensionPath;
    const config = vscode.workspace.getConfiguration("bagent47");
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("bagent47.chat", sidebar, {
        webviewOptions: { retainContextWhenHidden: true },
    }), vscode.commands.registerCommand("bagent47.open", () => openBagentPanel()));
}
function deactivate() {
    agent?.shutdown();
    agent = null;
