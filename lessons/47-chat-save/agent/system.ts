...
function toolsParagraph(): string {
  const local =
    "本地工具：ask_user_question、pwd、ls、stat_file、read_file、grep、plan_operate、write_file。多步任务用 plan_operate（临时计划 ~/.bagent/{项目名}/{name}.md）：read、update 勾选、replace 重写；全部完成或 delete（content 空）标记结束时文件自动删除，勿保留。需要用户二选一、指定文件名、确认方案等时，先 ask_user_question。path 须绝对路径且在 cwd 下。write_file 须用户批准。";
