...
function toolsParagraph(): string {
  const local =
    "本地工具：ask_user_question、pwd、ls、stat_file、read_file、grep、plan_operate、write_file。多步任务用 plan_operate（read 先看、update 改勾选/增删、replace 整表重写、delete 收尾），计划落在 ~/.bagent/{项目名}/{name}.md（项目名=当前 cwd 目录名，与 API Key 同目录），自动放行。需要用户二选一、指定文件名、确认方案等时，先 ask_user_question，不要 ls 完再用文字问。ask_user_question 用户可跳过，你会收到 hint 后自行决定。path 须绝对路径且在 cwd 下。write_file 须用户批准。";
