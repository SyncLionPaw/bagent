...
const baseToolDefinitions = [
    ...
    },
    {
        ...
        function: {
            name: "plan_operate",
            description: "管理任务计划（Markdown 待办），路径 ~/.bagent/{项目名}/{name}.md（项目名=当前 cwd 目录名，与用户 API Key 同目录），自动放行。改计划前先 read。new/replace 整表覆盖；update 合并（- [x] 勾选、- [-] 删项）；delete 删文件或条目。",
            parameters: {
                type: "object",
                properties: {
                    method: {
                        ...
                        description: "read：读取当前计划；new/replace：整表写入；update：合并/勾选/删单项；delete：删文件或删条目",
                    },
                    name: {
                        type: "string",
                    ...
                    content: {
                        type: "string",
                        description: "read 可空。new/replace：完整 Markdown 或待办行。update：- [ ]/- [x] 合并，- [-] 文本 删除该项。delete：空串删整份计划，否则写要删的条目",
                    },
                },
