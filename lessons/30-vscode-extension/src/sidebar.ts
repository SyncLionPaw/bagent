...
import { subscribeChat } from "./subscribe";
...
<body>
  <div class="hint">需先 npm run ch28:server</div>
  <div id="log"></div>
  <form id="f">
  ...
  resolveWebviewView(webviewView: vscode.WebviewView): void {
    ...
    webviewView.webview.onDidReceiveMessage(async (msg: { type: string; message?: string }) => {
      ...
      const baseUrl = vscode.workspace
        .getConfiguration("bagent")
        .get<string>("serverUrl", "http://localhost:3028");

      try {
        await subscribeChat(baseUrl, msg.message.trim(), (event: AgentEvent) => {
