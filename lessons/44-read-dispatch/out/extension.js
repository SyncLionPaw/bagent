...
async function openBagentPanel() {
    ...
    await vscode.commands.executeCommand("bagent44.chat.focus");
}
function activate(context) {
    const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? context.extensionPath;
    const config = vscode.workspace.getConfiguration("bagent44");
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("bagent44.chat", sidebar, {
        webviewOptions: { retainContextWhenHidden: true },
    }), vscode.commands.registerCommand("bagent44.open", () => openBagentPanel()));
}
function deactivate() {
    agent?.shutdown();
    agent = null;
