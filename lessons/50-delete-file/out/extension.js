...
async function openBagentPanel() {
    ...
    await vscode.commands.executeCommand("bagent50.chat.focus");
}
function activate(context) {
    const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? context.extensionPath;
    const config = vscode.workspace.getConfiguration("bagent50");
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("bagent50.chat", sidebar, {
        webviewOptions: { retainContextWhenHidden: true },
    }), vscode.commands.registerCommand("bagent50.open", () => openBagentPanel()));
}
function deactivate() {
    agent?.shutdown();
    agent = null;
