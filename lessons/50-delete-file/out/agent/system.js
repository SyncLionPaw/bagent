...
function toolsParagraph() {
    const local = "本地工具：ask_user_question、pwd、ls、stat_file、read_file、grep、plan_operate、write_file、str_replace、delete_file。read_file 返回带行号的文本（`行号|内容`），大文件用 offset/limit 按行读。小改动用 str_replace；整文件重写用 write_file；用户要求删单个文件用 delete_file（不可恢复）。多步任务用 plan_operate。path 须绝对路径且在 cwd 下。write_file、str_replace、delete_file 须用户批准。";
