...
async function openBagentPanel() {
    ...
    await vscode.commands.executeCommand("bagent48.chat.focus");
}
function activate(context) {
    const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? context.extensionPath;
    const config = vscode.workspace.getConfiguration("bagent48");
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("bagent48.chat", sidebar, {
        webviewOptions: { retainContextWhenHidden: true },
    }), vscode.commands.registerCommand("bagent48.open", () => openBagentPanel()));
}
function deactivate() {
    agent?.shutdown();
    agent = null;
