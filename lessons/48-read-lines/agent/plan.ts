...
function shouldRemovePlanFile(todos: PlanTodo[]): boolean {
  if (!todos.length) return true;
  return todos.every((t) => t.done);
}

function finishPlanFile(
  planName: string,
  method: PlanMethod,
  todos: PlanTodo[],
...
function commitTodos(
  ...
  const path = planFilePath(planName);
  ensureDir();
  writeFileSync(path, formatPlan(planName, todos), "utf-8");
  const { done, total } = planStats(todos);
  return JSON.stringify({
    ok: true,
    method,
...
export function snapshotFromToolOutput(output: string): PlanSnapshot | null {
  ...
  if (data.cleared === "all" || data.finished === true) {
    const todos = Array.isArray(data.todos) ? (data.todos as PlanTodo[]) : [];
    const { done, total } = planStats(todos);
